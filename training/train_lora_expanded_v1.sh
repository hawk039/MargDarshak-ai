...
DATA_FILE="$TRAIN_DIR/data/train_expanded_v1.jsonl"
CONFIG_FILE="$TRAIN_DIR/configs/qwen_1_5b_lora_expanded_v1.yaml"
ADAPTER_DIR="$TRAIN_DIR/adapters/marg_darshak_expanded_v1_lora"
OUTPUT_DIR="$TRAIN_DIR/outputs"
LOG_FILE="$OUTPUT_DIR/train_expanded_v1_$(date +"%Y%m%d_%H%M%S").log"
...
print_error() {
  echo "[train_lora_expanded_v1] $1" >&2
}

print_info() {
  echo "[train_lora_expanded_v1] $1"
}

if [[ ! -f "$DATA_FILE" ]]; then
  print_error "dataset not found at $DATA_FILE"
  print_error "expected a JSONL file at training/data/train_expanded_v1.jsonl"
  exit 1
fi
...
print_info "validating expanded_v1 dataset before training"
...
print_info "starting expanded_v1 LoRA training"
