...
DATA_FILE="$TRAIN_DIR/data/train.jsonl"
CONFIG_FILE="$TRAIN_DIR/configs/qwen_1_5b_lora.yaml"
...
if [[ ! -f "$DATA_FILE" ]]; then
  ...
  print_error "expected a JSONL file at training/data/train.jsonl"
  exit 1
fi
...
print_info "validating dataset before training"
