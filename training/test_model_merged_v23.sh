...
MERGED_V1_ADAPTER_DIR="$TRAIN_DIR/adapters/marg_darshak_merged_v1_lora"
...
print_error() {
  echo "[test_model_merged_v23] $1" >&2
}

print_info() {
  echo "[test_model_merged_v23] $1"
}
...
if [[ ! -d "$MERGED_V23_ADAPTER_DIR" ]]; then
  print_error "merged_v23 adapter directory not found: $MERGED_V23_ADAPTER_DIR"
  print_error "run training/train_lora_merged_v23.sh first"
  exit 1
fi

if [[ ! -f "$MERGED_V23_ADAPTER_DIR/adapters.safetensors" ]]; then
  print_error "merged_v23 adapter weights not found in $MERGED_V23_ADAPTER_DIR"
  print_error "expected adapters.safetensors after a successful merged_v23 training run"
  exit 1
fi

print_info "loading comparison models for merged_v23 testing"

MODEL_ID="$MODEL_ID" \
GITA_ADAPTER_DIR="$GITA_ADAPTER_DIR" \
MERGED_V1_ADAPTER_DIR="$MERGED_V1_ADAPTER_DIR" \
...
MERGED_V1_ADAPTER_DIR = os.environ["MERGED_V1_ADAPTER_DIR"]
...
if os.path.exists(os.path.join(MERGED_V1_ADAPTER_DIR, "adapters.safetensors")):
    merged_v1_model, merged_v1_tokenizer = load(MODEL_ID, adapter_path=MERGED_V1_ADAPTER_DIR)
    models.append(("Merged v1 LoRA", merged_v1_model, merged_v1_tokenizer))
