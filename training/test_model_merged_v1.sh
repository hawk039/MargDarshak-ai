...
MERGED_ADAPTER_DIR="$TRAIN_DIR/adapters/marg_darshak_merged_v1_lora"
MODEL_ID="${MODEL_ID:-mlx-community/Qwen2.5-1.5B-Instruct-4bit}"
PYTHON_BIN="${PYTHON_BIN:-python3}"

print_error() {
  echo "[test_model_merged_v1] $1" >&2
}

print_info() {
  echo "[test_model_merged_v1] $1"
}
...
if [[ ! -d "$MERGED_ADAPTER_DIR" ]]; then
  print_error "merged adapter directory not found: $MERGED_ADAPTER_DIR"
  print_error "run training/train_lora_merged_v1.sh first"
  exit 1
fi

if [[ ! -f "$MERGED_ADAPTER_DIR/adapters.safetensors" ]]; then
  print_error "merged adapter weights not found in $MERGED_ADAPTER_DIR"
  print_error "expected adapters.safetensors after a successful merged training run"
  exit 1
fi

print_info "loading comparison models for merged-v1 testing"

MODEL_ID="$MODEL_ID" GITA_ADAPTER_DIR="$GITA_ADAPTER_DIR" MERGED_ADAPTER_DIR="$MERGED_ADAPTER_DIR" "$PYTHON_BIN" - <<'PY'
import os
...
MERGED_ADAPTER_DIR = os.environ["MERGED_ADAPTER_DIR"]
SYSTEM_PROMPT = (
...
merged_model, merged_tokenizer = load(MODEL_ID, adapter_path=MERGED_ADAPTER_DIR)
models.append(("Merged LoRA", merged_model, merged_tokenizer))
