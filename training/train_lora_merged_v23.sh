...
DATA_FILE="$TRAIN_DIR/data/train_merged_v23.jsonl"
CONFIG_FILE="$TRAIN_DIR/configs/qwen_1_5b_lora_merged_v23.yaml"
ADAPTER_DIR="$TRAIN_DIR/adapters/marg_darshak_merged_v23_lora"
OUTPUT_DIR="$TRAIN_DIR/outputs"
LOG_FILE="$OUTPUT_DIR/train_merged_v23_$(date +"%Y%m%d_%H%M%S").log"
...
print_error() {
  echo "[train_lora_merged_v23] $1" >&2
}

print_info() {
  echo "[train_lora_merged_v23] $1"
}

if [[ ! -f "$DATA_FILE" ]]; then
  print_error "dataset not found at $DATA_FILE"
  print_error "expected a JSONL file at training/data/train_merged_v23.jsonl"
  exit 1
fi
...
print_info "validating merged_v23 dataset before training"
...
print_info "starting merged_v23 LoRA training"
